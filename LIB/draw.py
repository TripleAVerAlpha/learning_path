"""Класс TENSORFLOW с добавлением прокидки цвета"""
...
def add_edge(dot, src, dst):
    if not dot.get_edge(src, dst):
        dot.add_edge(pydot.Edge(src, dst, color=COLOR[1]))
  ...
  """
    ...
    for i, layer in enumerate(layers):
        ...
        if not expand_nested or not isinstance(
            ...
            node = pydot.Node(layer_id, label=label, color=COLOR[1], fontcolor=COLOR[2])
            dot.add_node(node)
    ...
    for layer in layers:
        ...
        for i, node in enumerate(layer._inbound_nodes):
            ...
            if node_key in model.network_nodes:
                for inbound_layer in nest.flatten(node.inbound_layers):
                    inbound_layer_id = str(id(inbound_layer))
                    if not expand_nested:
                        if len(dot.get_node(inbound_layer_id)) != 0:
